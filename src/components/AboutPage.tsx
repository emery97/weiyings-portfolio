  ...
  const timelineItems = [
    {
      ...
      description: "Began my journey in Information Technology, focusing on software development and system administration.",
      type: "education"
    },
    {
      year: "2023",
      title: "First Web Project",
      description: "Built my first website using HTML, CSS, and JavaScript. Discovered my passion for web development.",
      type: "project"
    },
    {
      year: "2023",
      title: "Learning React",
      description: "Started learning React and modern frontend development. Built several small applications to practice.",
      type: "skill"
    },
    {
      year: "2024",
      title: "Backend Development",
      description: "Expanded into backend development with Node.js and database management.",
      type: "skill"
    },
    {
      year: "2024",
      title: "Portfolio Project",
      description: "Created this portfolio website to showcase my skills and projects.",
      type: "project"
    }
  ];
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (timelineRef.current) {
        ...
        const newActiveItem = Math.min(Math.floor(rawIndex + 0.3), timelineItems.length - 1);
        ...
        >
          ...
          <div className="max-w-3xl mx-auto">
            <p className="text-lg text-muted-foreground mb-8">
              I'm a passionate IT student with a love for creating digital solutions that make a difference. 
              My journey in technology started with curiosity and has evolved into a dedication to continuous learning 
              and building innovative projects.
            </p>
            <p className="text-lg text-muted-foreground">
              When I'm not coding, you can find me exploring new technologies, contributing to open-source projects, 
              or thinking about how technology can solve real-world problems.
            </p>
          </div>
        </motion.div>
          ...
          <div ref={timelineRef} className="relative max-w-4xl mx-auto">
            ...
            <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 bg-border transform md:-translate-x-1/2">
              <motion.div
                ...
                initial={{ height: '0%' }}
                animate={{
                  height: `${(activeTimelineItem / Math.max(timelineItems.length - 1, 1)) * 100}%`
                }}
                transition={{ 
                  duration: 0.6,
                  ease: "easeInOut"
                ...
                <div className="absolute left-4 md:left-1/2 transform md:-translate-x-1/2 z-10">
                  <motion.div
                    className={`w-4 h-4 rounded-full border-4 border-background ${
                      index <= activeTimelineItem ? getTimelineItemColor(item.type) : 'bg-muted'
                    }`}
                  ...
                  {index <= activeTimelineItem && (
                    <motion.div
                      className={`absolute inset-0 rounded-full ${getTimelineItemColor(item.type)} opacity-30`}
                      initial={{ scale: 1 }}
                  ...
                  >
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Badge 
                          variant="secondary" 
                          className={`text-xs ${getTimelineItemColor(item.type)} text-white`}
                        >
                          {item.type}
                        </Badge>
                      </div>
